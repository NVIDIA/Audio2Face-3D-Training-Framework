...
#
############################################################################
# Modified from charsiu
# Copyright (c) 2021 jzhu
#
# See https://github.com/lingjzhu/charsiu/blob/main/LICENSE for details
############################################################################
#
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
processors = {"zh": CharsiuPreprocessor_zh, "en": CharsiuPreprocessor_en}


class charsiu_aligner:
    def __init__(self, lang="en", sampling_rate=16000, device=None, recognizer=None, processor=None, resolution=0.01):
        self.lang = lang
...
class charsiu_attention_aligner(charsiu_aligner):
    ...
    def align(self, audio, text):
        ...
        batch = {"input_values": audio, "labels": torch.tensor(phone_ids).unsqueeze(0).long().to(self.device)}
...
class charsiu_predictive_aligner(charsiu_aligner):
    def __init__(self, aligner, temperature=1.0, **kwargs):
    ...
    def align_probs(self, audio: str | np.ndarray, sr: int | None = None):
        audio = self.charsiu_processor.audio_preprocess(audio, sr=(sr if sr is not None else self.sr))
        audio = torch.Tensor(audio).unsqueeze(0).to(self.device)
        with torch.no_grad():
            logits = self.aligner(audio, return_logits=True)
        probs = torch.softmax(logits.squeeze() / self.temperature, dim=-1)
...
if __name__ == "__main__":
    ...
    charsiu.serve(
        audio="./local/SSB00050015_16k.wav",
        text="经广州日报报道后成为了社会热点。",
