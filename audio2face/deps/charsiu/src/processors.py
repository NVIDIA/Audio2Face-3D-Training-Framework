...
# WARNING: Removed dependency on nltk, g2p_en, g2pM
###################################################
...
class CharsiuPreprocessor:
    ...
    def audio_preprocess(self, audio, sr=None):
        """
        ...
        sr : int, optional
            Original audio sampling rate, used if audio is np.ndarray
        ...
        target_sr = 16000  # W2V requirement

        if type(audio) == str:
            features, _ = librosa.core.load(audio, sr=target_sr)  # resample to target_sr
        elif isinstance(audio, np.ndarray):
            features = audio
            assert sr == target_sr
        else:
            raise Exception("The input must be a path or a numpy array!")
        return self.processor(features, sampling_rate=target_sr, return_tensors="pt").input_values.squeeze()
...
class CharsiuPreprocessor_en(CharsiuPreprocessor):
    def __init__(self):
        ...
        # self.g2p = G2p()  # WARNING: Removed dependency on nltk, g2p_en, g2pM
        self.sil = "[SIL]"
...
class CharsiuPreprocessor_zh(CharsiuPreprocessor_en):
    def __init__(self):
        ...
        self.processor = Wav2Vec2Processor(feature_extractor=feature_extractor, tokenizer=tokenizer)
        # self.g2p = G2pM() # WARNING: Removed dependency on nltk, g2p_en, g2pM
        self.sil = "[SIL]"
        self.sil_idx = self.mapping_phone2id(self.sil)
        # self.punctuation = set('.,!?。，！？、')
        self.punctuation = set()
        # Pinyin tables
        self.consonant_list = set(
