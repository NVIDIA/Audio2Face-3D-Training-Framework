...
class ForwardSumLoss(torch.nn.Module):
    ...
    def forward(self, attn_logprob, text_lens, mel_lens):
        ...
        for bid in range(attn_logprob.shape[0]):
            ...
            cost = self.CTCLoss(
                curr_logprob, target_seq, input_lengths=mel_lens[bid : bid + 1], target_lengths=text_lens[bid : bid + 1]
            )
            cost_total += cost
...
class RNN(nn.Module):
    def __init__(self, hidden_dim, out_dim):
        ...
        self.linear = nn.Sequential(nn.Linear(2 * hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, out_dim))
...
class Wav2Vec2ForAttentionAlignment(Wav2Vec2ForPreTraining):
    ...
    def forward(
        ...
        return CausalLMOutput(loss=loss, logits=energy, hidden_states=outputs.hidden_states, attentions=None)
