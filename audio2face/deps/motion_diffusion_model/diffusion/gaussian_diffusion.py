...
#
############################################################################
# Modified from motion-diffusion-model
# Copyright (c) 2022 Guy Tevet
#
# See https://github.com/GuyTevet/motion-diffusion-model/blob/main/LICENSE for details
############################################################################
#
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        **kargs,
        ...
        self.loss = lambda a, b: torch.where(
            torch.abs(a - b) < 1.0, 0.5 * (a - b) ** 2, torch.abs(a - b) - 0.5
        ).mean()  # SmoothL1Loss
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        assert t.shape == (B,)
        if (
            model.model.inference_mode == "streaming_stateless"
            or model.model.inference_mode == "streaming_stateless_output_delta"
        ):
            model_output, h_gru_all = model(x, self._scale_timesteps(t), **model_kwargs)
        else:
            model_output = model(x, self._scale_timesteps(t), **model_kwargs)  # modify to add gru_h in input/output
    ...
    def p_sample(
        ...
        """
        # predict the mean of x_(t-1) from x_t by denoising
        ...
        noise = th.randn_like(x) if sampling_noise is None else sampling_noise  # Batch size, nFrames, 88831
        # print('const_noise', const_noise)
        if const_noise:
            noise = noise[[0]].repeat(x.shape[0], 1, 1)  # same noise across batch
        ...
        if model.inference_mode == "streaming_stateless" or model.inference_mode == "streaming_stateless_output_delta":
            return {"sample": sample, "pred_xstart": out["pred_xstart"], "h_gru_all": out["h_gru_all"]}
        else:
    ...
    def p_sample_loop(
        ...
        if dump_steps is not None:
            return dump
        if model.inference_mode == "streaming_stateless":
            return final["sample"], final["h_gru_all"]
        elif model.inference_mode == "streaming_stateless_output_delta":
            # final is (output + mean_geometry). To get absolute pos_delta: (final - mean_geometry[interpolated]) * scale[interpolated]
            actor_vec = model_kwargs["actor_vec"]
            output = final["sample"]

            scale = actor_vec @ model.scale  # B,
            # offset = actor_vec @ model.offset # B,3
            tongue_data_scale = actor_vec @ model.tongue_data_scale  # B,
            # tongue_data_offset = actor_vec @ model.tongue_data_offset # B,3
            jaw_data_scale = actor_vec @ model.jaw_data_scale  # B,
            eye_data_scale = actor_vec @ model.eyeball_data_scale  # B,
            mean_geometry = actor_vec @ model.mean_geometry

            # Skin part
            skin_vert = (output[:, :, : model.skin_dim] - mean_geometry[:, None, : model.skin_dim]) * scale[
                :, None, None
            ]
            B, L, _ = skin_vert.shape
            # skin_vert = (skin_vert.view(B,L, -1, 3) + offset[:,None,None,:]).view(B,L,-1)
            output[:, :, : model.skin_dim] = skin_vert

            # Tongue part
            tongue_vert = (
                output[:, :, model.skin_dim : (model.skin_dim + model.tongue_dim)]
                - mean_geometry[:, None, model.skin_dim : (model.skin_dim + model.tongue_dim)]
            ) * tongue_data_scale[:, None, None]
            # tongue_vert = (tongue_vert.view(B,L, -1, 3) + tongue_data_offset[:,None,None,:]).view(B,L,-1)
            output[:, :, model.skin_dim : (model.skin_dim + model.tongue_dim)] = tongue_vert

            # #other parts
            output[
                :,
                :,
                (model.skin_dim + model.tongue_dim) : (model.skin_dim + model.tongue_dim + model.jaw_dim),
            ] = (
                output[
                    :,
                    :,
                ...
                * jaw_data_scale[:, None, None]
            )
            output[:, :, -model.eye_dim :] = output[:, :, -model.eye_dim :] * eye_data_scale[:, None, None]

            return output, final["h_gru_all"]
        else:
            return final["sample"]
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            initial_noise = noise[:, -1]  # broadcast the noise along batch dim.
            img = initial_noise
        else:
            img = th.randn(*shape, device=device)  # Batch size, nFrames, 88831
        ...
        for i in indices:
            # This raise the tracing warning and create constant t!
            ...
            with th.no_grad():
                ...
                # one denoising steps, sample x_(t-1) from xt
                out = sample_fn(
                    model,
                    img,
                    t,
                ...
                yield out
                img = out["sample"]  # update the sample to be denoised in next iteration from xt by x_(t-1)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, tongue_weight=None, jaw_weight=None):
        """
        ...
        if noise is None:
            noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start, t, noise=noise)  # 1, nFrame (30fps), 3V
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            #             pred_vel, torch.zeros(pred_vel.shape, device=pred_vel.device), mask[:, :, :, 1:]
            #         )
            ...
            #         pred_target, ref_target, model_kwargs["y"], model.all_goal_joint_names
            #     )
            ...
            # Adjust tongue/jaw loss weighting
            weight = torch.ones_like(target)  # Start with all weights equal to 1
            weight[:, :, model.model.skin_dim : (model.model.skin_dim + model.model.tongue_dim)] = tongue_weight
            weight[
                :,
                :,
                (model.model.skin_dim + model.model.tongue_dim) : (
                    model.model.skin_dim + model.model.tongue_dim + model.model.jaw_dim
                ),
            ] = jaw_weight
            total_loss = self.loss(target * weight, model_output * weight)
    ...
    def velocity_consistency_loss_humanml3d(self, target, model_output):
        ...
        for i in range(21):
            plt.plot(
                np.linalg.norm(
                    calc_vel_from_xyz[:, i * 3 : (i + 1) * 3, :, :].cpu().detach().numpy().reshape((3, -1)), axis=0
                ),
                label="Calc Vel",
            )
            plt.plot(
                np.linalg.norm(
                    velocity_from_vector[:, i * 3 : (i + 1) * 3, :, :].cpu().detach().numpy().reshape((3, -1)), axis=0
                ),
                label="Vector Vel",
            )
